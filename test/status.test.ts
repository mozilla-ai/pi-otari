...
describe("lifecycle UI", () => {
  it("shows routing status only for Otari models", async () => {
    const { handlers } = harness({ models: [], diagnostics: [], discoverySource: "none" });
    const ctx = context();
    await handlers.get("model_select")?.({ model: { provider: "otari", id: "anthropic:claude" } }, ctx);
    expect(ctx.ui.setStatus).toHaveBeenCalledWith("pi-otari", "Otari → anthropic:claude");
    await handlers.get("model_select")?.({ model: { provider: "openai", id: "gpt" } }, ctx);
  ...
  it("notifies once when no models are available", async () => {
    ...
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("OTARI_MODELS"), "warning");
  });
  ...
  it("does not show a success notification", async () => {
    ...
    expect(ctx.ui.setStatus).toHaveBeenCalledWith("pi-otari", "Otari → mzai:model");
  });
});
