...
A failed scheduled run opens an issue labelled `live-check` with the failing stage and a link to the run. Further failures update that issue's body with the latest run and the consecutive count rather than opening new issues, and every failing run adds a comment with its date, run link, failing stage, and full output, so the history of an outage stays in one place. The first passing scheduled run closes the issue. After five consecutive failures the scheduled check runs on Mondays only until it passes again. Manual runs leave the issue alone unless started with the `as_scheduled` option, which exists to test this flow.
