...
function validModel(value: unknown): value is OtariModel {
  ...
  if (
    ...
    !MODEL_SOURCES.has(model.source as ModelSource)
  )
    return false;
  ...
  if (
    ...
    return false;
  if (
    !optionalPositiveNumber(model.contextWindow) ||
    !optionalPositiveNumber(model.maxTokens)
  )
    return false;
...
export async function readModelCache(
  agentDir: string,
): Promise<{ cache?: ModelCache; diagnostic?: Diagnostic }> {
  try {
    const parsed = JSON.parse(
      await readFile(cachePath(agentDir), "utf8"),
    ) as Partial<ModelCache>;
